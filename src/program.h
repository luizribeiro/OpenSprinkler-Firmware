...
public:
  ...
  // monthly:  days[0][0..5] stores the day of the month (32 means last day of
  // month) interval: days[0] stores the interval (0 to 255), days[1] stores the
  // starting day remainder (0 to 254)
  byte days[2];
...
public:
  ...
  static byte station_qid[]; // this array stores the queue element index for
                             // each scheduled station
  ...
  static RuntimeQueueStruct *
  enqueue(); // this returns a pointer to the next available slot in the queue
  ...
  static void
  drem_to_relative(byte days[2]); // absolute to relative reminder conversion
