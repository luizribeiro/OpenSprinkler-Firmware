...
#define MQTT_DEFAULT_PORT                                                      \
  1883 // Default port for MQTT. Can be overwritten through App config
#define MQTT_MAX_HOST_LEN 50 // Note: App is set to max 50 chars for broker name
#define MQTT_MAX_USERNAME_LEN                                                  \
  32 // Note: App is set to max 32 chars for username
#define MQTT_MAX_PASSWORD_LEN                                                  \
  32                       // Note: App is set to max 32 chars for password
#define MQTT_MAX_ID_LEN 16 // MQTT Client Id to uniquely reference this unit
#define MQTT_RECONNECT_DELAY                                                   \
  120 // Minumum of 60 seconds between reconnect attempts
...
char OSMqtt::_id[MQTT_MAX_ID_LEN + 1] = {
    0}; // Id to identify the client to the broker
char OSMqtt::_host[MQTT_MAX_HOST_LEN + 1] = {
    0}; // IP or host name of the broker
char OSMqtt::_username[MQTT_MAX_USERNAME_LEN + 1] = {
    0}; // username to connect to the broker
char OSMqtt::_password[MQTT_MAX_PASSWORD_LEN + 1] = {
    0};                                // password to connect to the broker
...
// Start the MQTT service and connect to the MQTT broker using the stored
// configuration.
void OSMqtt::begin(void) {
  ...
  // JSON configuration settings in the form of {"en":0|1,"host":"server_name|IP
  // address","port":1883,user:"",pass:""}
...
void OSMqtt::begin(const char *host, int port, const char *username,
                   const char *password, bool enabled) {
...
// Regularly call the loop function to ensure "keep alive" messages are sent to
// the broker and to reconnect if needed.
void OSMqtt::loop(void) {
  ...
  // Only attemp to reconnect every MQTT_RECONNECT_DELAY seconds to avoid
  // blocking the main loop
  if (!_connected() &&
      (millis() - last_reconnect_attempt >= MQTT_RECONNECT_DELAY * 1000UL)) {
...
static void _mqtt_connection_cb(struct mosquitto *mqtt_client, void *obj,
                                int reason) {
  ...
  if (reason == 0) {
    int rc = mosquitto_publish(mqtt_client, NULL, MQTT_AVAILABILITY_TOPIC,
                               strlen(MQTT_ONLINE_PAYLOAD), MQTT_ONLINE_PAYLOAD,
                               0, true);
...
static void _mqtt_disconnection_cb(struct mosquitto *mqtt_client, void *obj,
                                   int reason) {
...
static void _mqtt_log_cb(struct mosquitto *mqtt_client, void *obj, int level,
                         const char *message) {
...
int OSMqtt::_init(void) {
  ...
  mosquitto_will_set(mqtt_client, MQTT_AVAILABILITY_TOPIC,
                     strlen(MQTT_OFFLINE_PAYLOAD), MQTT_OFFLINE_PAYLOAD, 0,
                     true);
...
int OSMqtt::_connect(void) {
  ...
  // Allow 10ms for the Broker's ack to be received. We need this on start-up so
  // that the connection is registered before we attempt to send our first
  // NOTIFY_REBOOT notification.
  usleep(10000);
...
int OSMqtt::_publish(const char *topic, const char *payload) {
  int rc = mosquitto_publish(mqtt_client, NULL, topic, strlen(payload), payload,
                             0, false);
  if (rc != MOSQ_ERR_SUCCESS) {
    DEBUG_LOGF("MQTT Publish: Failed (%s)\r\n", mosquitto_strerror(rc));
    return MQTT_ERROR;
...
int OSMqtt::_loop(void) { return mosquitto_loop(mqtt_client, 0, 1); }
