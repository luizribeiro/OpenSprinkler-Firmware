...
void initialiseEpoch() {
  ...
  epochMilli =
      (uint64_t)tv.tv_sec * (uint64_t)1000 + (uint64_t)(tv.tv_usec / 1000);
...
void write_to_file(const char *fn, const char *data, ulong size, ulong pos,
                   bool trunc) {
...
void file_copy_block(const char *fn, ulong from, ulong to, ulong len,
                     void *tmp) {
