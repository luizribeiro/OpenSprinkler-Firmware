...
struct ConStatus {
  byte enabled : 1;      // operation enable (when set, controller operation is
                         // enabled)
  ...
  byte
      sensor1_active : 1; // sensor1 active bit (when set, sensor1 is activated)
  byte
      sensor2_active : 1; // sensor2 active bit (when set, sensor2 is activated)
...
public:
  ...
  static byte
      station_bits[]; // station activation bits. each byte corresponds to a
                      // board (8 stations) first byte-> master controller,
                      // second byte-> ext. board 1, and so on
  ...
  static ulong
      raindelay_on_lasttime; // time when the most recent rain delay started
  ...
  static ulong
      checkwt_success_lasttime; // time when weather check was successful
  static ulong
      powerup_lasttime; // time when controller is powered up most recently
  ...
  static void
  attribs_save(); // repackage attrib bits and save (backward compatibility)
  static void
  attribs_load(); // load and repackage attrib bits (backward compatibility)
  static uint16_t
  parse_rfstation_code(RFStationData *data, ulong *on,
                       ulong *off); // parse rf code into on/off/time sections
  static void switch_rfstation(RFStationData *data,
                               bool turnon); // switch rf station
  static void switch_remotestation(RemoteStationData *data,
                                   bool turnon); // switch remote station
  static void switch_gpiostation(GPIOStationData *data,
                                 bool turnon); // switch gpio station
  static void switch_httpstation(HTTPStationData *data,
                                 bool turnon); // switch http station
  ...
  static void detect_binarysensor_status(
      ulong); // update binary (rain, soil) sensor status
  ...
  static byte set_station_bit(byte sid,
                              byte value); // set station bit of one station
                                           // (sid->station index, value->0/1)
  static void switch_special_station(byte sid,
                                     byte value); // swtich special station
  static void clear_all_station_bits();           // clear all station bits
  static void
  apply_all_station_bits(); // apply all station bits (activate/deactive values)

  static int8_t send_http_request(uint32_t ip4, uint16_t port, char *p,
                                  void (*callback)(char *) = NULL,
                                  uint16_t timeout = 3000);
  static int8_t send_http_request(const char *server, uint16_t port, char *p,
                                  void (*callback)(char *) = NULL,
                                  uint16_t timeout = 3000);
