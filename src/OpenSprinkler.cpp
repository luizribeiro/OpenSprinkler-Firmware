...
void OpenSprinkler::get_station_data(byte sid, StationData *data) {
  file_read_block(STATIONS_FILENAME, data, (uint32_t)sid * sizeof(StationData),
                  sizeof(StationData));
}
...
void OpenSprinkler::get_station_name(byte sid, char tmp[]) {
  ...
  file_read_block(STATIONS_FILENAME, tmp,
                  (uint32_t)sid * sizeof(StationData) +
                      offsetof(StationData, name),
                  STATION_NAME_SIZE);
}
...
void OpenSprinkler::set_station_name(byte sid, char tmp[]) {
  ...
  tmp[STATION_NAME_SIZE] = 0;
  file_write_block(STATIONS_FILENAME, tmp,
                   (uint32_t)sid * sizeof(StationData) +
                       offsetof(StationData, name),
                   STATION_NAME_SIZE);
}
...
void OpenSprinkler::attribs_load() {
  ...
  for (bid = 0; bid < MAX_NUM_BOARDS; bid++) {
    for (s = 0; s < 8; s++, sid++) {
      ...
      file_read_block(
          STATIONS_FILENAME, &ty,
          (uint32_t)sid * sizeof(StationData) + offsetof(StationData, type), 1);
...
byte OpenSprinkler::set_station_bit(byte sid, byte value) {
  ...
  if (value) {
    if ((*data) & mask)
      ...
      (*data) = (*data) | mask;
      engage_booster =
          true; // if bit is changing from 0 to 1, set engage_booster
    ...
    if (!((*data) & mask))
      ...
      if (hw_type == HW_TYPE_LATCH) {
        engage_booster =
            true; // if LATCH controller, engage booster when bit changes
      }
