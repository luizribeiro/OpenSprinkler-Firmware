...
void ProgramData::dequeue(byte qid) {
  if (qid >= nqueue)
    return;
  if (qid < nqueue - 1) {
    queue[qid] = queue[nqueue - 1]; // copy the last element to the dequeud
                                    // element to fill the space
    if (station_qid[queue[qid].sid] ==
        nqueue - 1) // fix queue index if necessary
...
void ProgramData::load_count() { nprograms = file_read_byte(PROG_FILENAME, 0); }

/** Save program count to program file */
void ProgramData::save_count() { file_write_byte(PROG_FILENAME, 0, nprograms); }
...
void ProgramData::read(byte pid, ProgramStruct *buf) {
  ...
  file_read_block(PROG_FILENAME, buf, 1 + (ulong)pid * PROGRAMSTRUCT_SIZE,
                  PROGRAMSTRUCT_SIZE);
}
...
byte ProgramData::add(ProgramStruct *buf) {
  if (nprograms >= MAX_NUM_PROGRAMS)
    return 0;
  file_write_block(PROG_FILENAME, buf,
                   1 + (ulong)nprograms * PROGRAMSTRUCT_SIZE,
                   PROGRAMSTRUCT_SIZE);
  nprograms++;
  save_count();
  return 1;
...
byte ProgramData::del(byte pid) {
  ...
  for (; pos < 1 + (ulong)nprograms * PROGRAMSTRUCT_SIZE;
       pos += PROGRAMSTRUCT_SIZE) {
    file_copy_block(PROG_FILENAME, pos, pos - PROGRAMSTRUCT_SIZE,
                    PROGRAMSTRUCT_SIZE, tmp_buffer);
  }
  nprograms--;
  save_count();
  return 1;
...
byte ProgramData::set_flagbit(byte pid, byte bid, byte value) {
  if (pid >= nprograms)
    return 0;
  byte flag =
      file_read_byte(PROG_FILENAME, 1 + (ulong)pid * PROGRAMSTRUCT_SIZE);
  if (value)
    flag |= (1 << bid);
  else
    flag &= (~(1 << bid));
...
byte ProgramStruct::check_day_match(time_t t) {
  ...
  byte weekday_t =
      (ti->tm_wday + 1) % 7; // tm_wday ranges from [0,6] with Sunday being 0
  ...
  case PROGRAM_TYPE_INTERVAL:
    ...
    if (((t / SECS_PER_DAY) % days[1]) != days[0])
      return 0;
    break;
  }
...
byte ProgramStruct::check_match(time_t t) {
    ...
    if (starttime_type) {
      ...
      for (byte i = 0; i < MAX_NUM_STARTTIMES; i++) {
        if (current_minute == starttime_decode(starttimes[i]))
          return 1; // if curren_minute matches any of the given start time,
                    // return 1
      }
      ...
      // otherwise, current_minute must be larger than start time, and interval
      // must be non-zero
  ...
  // to proceed, program has to be repeating type, and interval and repeat must
  // be non-zero
  if (starttime_type || !interval)
    return 0;
...
// convert absolute remainder (reference time 1970 01-01) to relative remainder
// (reference time today) absolute remainder is stored in flash, relative
// remainder is presented to web
