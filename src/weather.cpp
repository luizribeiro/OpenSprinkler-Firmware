...
// The weather function calls getweather.py on remote server to retrieve weather
// data the default script is WEATHER_SCRIPT_HOST/weather?.py
...
static void getweather_callback(char *buffer) {
  ...
  if (wt_errCode == 0 &&
      findKeyVal(p, tmp_buffer, TMP_BUFFER_SIZE, PSTR("scale"), true)) {
    v = atoi(tmp_buffer);
...
void GetWeather() {
  ...
  bf.emit_p(PSTR("$D?loc=$O&wto=$O&fwv=$D"), (int)os.iopts[IOPT_USE_WEATHER],
            SOPT_LOCATION, SOPT_WEATHER_OPTS, (int)os.iopts[IOPT_FW_VERSION]);
  ...
  *dst = *src;
  ...
  int ret = os.send_http_request(host, ether_buffer,
                                 getweather_callback_with_peel_header);
  if (ret != HTTP_RQT_SUCCESS) {
    if (wt_errCode < 0)
      wt_errCode = ret;
    // if wt_errCode > 0, the call is successful but weather script may return
    // error
  }
}
