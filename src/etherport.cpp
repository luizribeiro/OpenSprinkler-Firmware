...
EthernetServer::EthernetServer(uint16_t port) : m_port(port), m_sock(0) {}

EthernetServer::~EthernetServer() { close(m_sock); }
...
EthernetClient EthernetServer::available() {
  ...
  if (FD_ISSET(m_sock, &sock_set)) {
    ...
    if ((client_sock = accept(m_sock, (struct sockaddr *)&cli_addr, &clilen)) <=
        0)
...
EthernetClient::EthernetClient() : m_sock(0), m_connected(false) {}

EthernetClient::EthernetClient(int sock) : m_sock(sock), m_connected(true) {}

EthernetClient::~EthernetClient() { stop(); }
...
EthernetClient::operator bool() { return m_sock != 0; }

// read data from the client into the buffer provided
//	This function will block until either data is received OR a timeout
//happens. 	If an error occurs or a timeout happens, we set the disconnect flag
//on the socket 	and return 0;
